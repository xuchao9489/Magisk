...
ElfImg::getSymbOffset(std::string_view name, uint32_t gnu_hash, uint32_t elf_hash) const {
    if (auto offset = GnuLookup(name, gnu_hash); offset > 0) {
        LOGD("zygisk: found %s %p in %s in dynsym by gnuhash\n", name.data(),
             reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else if (offset = ElfLookup(name, elf_hash); offset > 0) {
        LOGD("zygisk: found %s %p in %s in dynsym by elfhash\n", name.data(),
             reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else if (offset = LinearLookup(name); offset > 0) {
        LOGD("zygisk: found %s %p in %s in symtab by linear lookup\n", name.data(),
             reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else {
        return 0;
...
bool ElfImg::findModuleBase() {
    ...
    if (!found) {
        LOGE("zygisk: failed to read load address for %s\n", elf.data());
        fclose(maps);
        return false;
    }
    ...
    LOGD("zygisk: get module base %s: %lx\n", elf.data(), load_addr);
